...
if (isset($_POST['add_agenda'])) {
    ...
    if (empty($errors)) {
        ...
        $sql = "UPDATE agendas SET fecha='{$fecha}', actividad='{$actividad}', area_responsable='{$area_responsable}', responsable='{$responsable}', area_supervisor='{$area_supervisor}', supervisor='{$supervisor}', fecha_limite='{$fecha_limite}', fecha_entrega='{$fecha_entrega}', entregables='{$entregables}', observaciones='{$observaciones}' WHERE id='{$db->escape($id)}'";
...
<div class="row">
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            <form method="post" action="edit_agenda.php?id=<?php echo (int)$e_agenda['id']; ?>" enctype="multipart/form-data">
                <div class="row">
                    <div class="col-md-2">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" id="responsable_editar" name="responsable">
                                <?php foreach ($trabajadores as $trabajador) : ?>
                                    <option <?php if ($trabajador['id'] === $e_agenda['responsable']) echo 'selected="selected"'; ?> value="<?php echo $trabajador['id']; ?>"><?php echo ucwords($trabajador['nombre'] . " " . $trabajador['apellidos']); ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" id="supervisor_editar" name="supervisor">
                                <?php foreach ($trabajadores2 as $trabajador2) : ?>
                                    <option <?php if ($trabajador2['id'] === $e_agenda['supervisor']) echo 'selected="selected"'; ?> value="<?php echo $trabajador2['id']; ?>"><?php echo ucwords($trabajador2['nombre'] . " " . $trabajador2['apellidos']); ?></option>
                                <?php endforeach; ?>
                            </select>
