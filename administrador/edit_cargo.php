...
<?php
$areas = find_all('area');
$e_cargo = find_by_id('cargos', (int)$_GET['id']);
if (!$e_cargo) {
...
if (isset($_POST['update'])) {
    ...
    if (empty($errors)) {
        ...
        $query .= "WHERE ID='{$db->escape($e_cargo['id'])}'";
        $result = $db->query($query);
        if ($result && $db->affected_rows() === 1) {
            //sucess
            $session->msg('s', "Cargo actualizado! ");
            redirect('edit_cargo.php?id=' . (int)$e_cargo['id'], false);
        } else {
            //failed
...
<div class="login-page">
    ...
    <form method="post" action="edit_cargo.php?id=<?php echo (int)$e_cargo['id']; ?>" class="clearfix">
        ...
        <div class="form-group">
            ...
            <select class="form-control" name="area">
                <?php foreach ($areas as $area) : ?>
                    <option <?php if ($area['id'] === $e_cargo['id_area']) echo 'selected="selected"'; ?> value="<?php echo $area['id']; ?>"><?php echo ucwords($area['nombre_area']); ?></option>
                <?php endforeach; ?>
            </select>
        </div>
